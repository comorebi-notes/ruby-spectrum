...
data.each_with_index do |data_unit, index|
  ...
  spectrum_plot_params = {
    ...
    title:      "spectrum (#{data_unit[1]} - #{data_unit[2].ceil(1)}s)"
  }

  plotter = GNUPlotter.new(spectrum, spectrum_plot_params, 4096, 96)
  plotter.plot
end

system 'convert -delay 10 -loop 0 tmp/gnuplot/spectrum_*.png tmp/gnuplot/spectrum.gif'
system 'ffmpeg -r 10 -i tmp/gnuplot/spectrum.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" tmp/gnuplot/spectrum.tmp.mp4'
